...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- DICCIONARIO PARA i18n ---
    const uiStrings = {
        es: { points: "Puntos:", main_title: "El Nuevo Juego de Valeria", game_math: "Reto Matemático", game_guess_word: "Adivina la Palabra", game_shop: "Tienda", game_closet: "Mi Colección", back_button: "&larr; Volver", attempts_left: "Intentos restantes:", settings_title: "⚙️ Ajustes", settings_appearance: "Apariencia", settings_dark_mode: "Modo Oscuro", settings_select_difficulty: "Selecciona dificultad:", settings_easy: "Fácil", settings_medium: "Medio", settings_hard: "Difícil", settings_select_theme: "Selecciona tema:", check_answer: "Comprobar", current_difficulty: "Dificultad:", closet_subtitle: "Selecciona tu avatar activo" },
        en: { points: "Points:", main_title: "Valeria's New Game", game_math: "Math Challenge", game_guess_word: "Guess the Word", game_shop: "Shop", game_closet: "My Collection", back_button: "&larr; Back", attempts_left: "Attempts left:", settings_title: "⚙️ Settings", settings_appearance: "Appearance", settings_dark_mode: "Dark Mode", settings_select_difficulty: "Select difficulty:", settings_easy: "Easy", settings_medium: "Medium", settings_hard: "Hard", settings_select_theme: "Select a theme:", check_answer: "Check", current_difficulty: "Difficulty:", closet_subtitle: "Select your active avatar" }
    };
    ...
    const ui = {
        init() {
            ...
            this.elements.backButtons.forEach(button => {
                button.addEventListener('click', () => this.showView('main-menu-view'));
            });
        },
        showView(viewId) {
        ...
        updateTexts() {
            ...
            document.querySelectorAll('[data-i18n-key]').forEach(el => {
                ...
                if (uiStrings[lang][key]) el.innerHTML = uiStrings[lang][key];
            });
        }
    };
    ...
    const persistence = {
        ...
        load() {
            ...
            if (savedState) {
                ...
                // Merge inteligente para futuras actualizaciones
    ...
    const guessWordGame = {
        ...
        renderBoxes() {
            this.elements.boxes.innerHTML = '';
            for (const letter of state.guessWordState.word) {
                const box = document.createElement('div');
                box.className = 'letter-box';
                if (state.guessWordState.guessedLetters.has(letter)) box.textContent = letter;
                this.elements.boxes.appendChild(box);
            }
        },
        renderKeyboard() {
        ...
        handleGuess(letter) {
            if (state.guessWordState.attempts <= 0 || state.guessWordState.guessedLetters.has(letter)) return;
            state.guessWordState.guessedLetters.add(letter);
            this.elements.keyboard.querySelector(`button:nth-child(${letter.charCodeAt(0) - 64})`).disabled = true;
            if (state.guessWordState.word.includes(letter)) {
                this.renderBoxes();
            } else {
                state.guessWordState.attempts--;
                this.updateAttempts();
            }
